...
let ItemMarkdownBuilder = function (markdown) {
  ...
  markdown = markdown.replace(/^-{3,}\s*$/gm, "<hr>");
  markdown = markdown.replace(/^_{3,}\s*$/gm, "<hr>");
  ...
  markdown = markdown.replace(/(<ul><li>.*<\/li><\/ul>)/gim, function (match) {
    return match.replace(/<\/li><li>/g, "</li><li>");
  });

  // Convert ordered lists
  markdown = markdown.replace(/^\d+\. (.*$)/gim, "<ol><li>$1</li></ol>");
  markdown = markdown.replace(/(<ol><li>.*<\/li><\/ol>)/gim, function (match) {
    return match.replace(/<\/li><li>/g, "</li><li>");
  });

  // Convert nested lists
  markdown = markdown.replace(/^\s*\*\s(.*$)/gm, function (match) {
    return "<ul><li>" + match.replace(/^\s*\*\s/, "") + "</li></ul>";
  });
  markdown = markdown.replace(/^\s*\d+\.\s(.*$)/gm, function (match) {
  ...
  markdown = markdown.replace(
    ...
    function (match, p1) {
      ...
      const tableRows = rows
        ...
        .join("");
      return `<table><thead><tr>${headers}</tr></thead><tbody>${tableRows}</tbody></table>`;
    }
  );
  ...
  //horizontal rule
  markdown = markdown.replace(/-{3,}/g, "<hr>");
  //ordered lists
  markdown = markdown.replace(/^\d+\.\s/gm, "<ol><li>");
  //unordered lists
  markdown = markdown.replace(/^\*\s/gm, "<ul><li>");
  //unordered list with -
  markdown = markdown.replace(/^-/gm, "<ul><li>");

  // Convert newlines to <br> (for simple line breaks)
  markdown = markdown.replace(/(\S)\n(\S)/g, "$1<br>$2"); // Convert line breaks between text
