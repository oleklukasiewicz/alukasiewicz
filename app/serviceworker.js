...
const networkOnlyResources = ["/v1/"];
const alwaysFreshResources = ["/item/"];
...
const cacheListCSS = ["/css/global.css", "/css/item.css"];
const cacheListJS = [
  "/item/storage.js",
  "/js/item.js",
  "/js/prerender.js",
...
const cacheList = cacheListApp.concat(
  cacheListCSS,
  cacheListJS,
  cacheListIcons,
  cacheListImages
...
const isAlwaysFreshResource = (req) =>
  alwaysFreshResources.find((resource) => req.includes(resource));
const isNetworkOnlyResource = (req) =>
  networkOnlyResources.find((resource) => req.includes(resource));
...
const cacheResource = (resource) =>
  caches
    ...
    .then((cache) =>
      caches
        .match(resource, { ignoreVary: true })
        .then((res) => (!res ? cache.add(resource) : null))
    );
const removeResourceFromCache = (resource) =>
  caches
    .open(cacheName)
    .then((cache) =>
      ...
      event.waitUntil(
        (async function () {
          ...
          if (
            networkResponse &&
            (putInCache == true ||
              (putInCache == "only-if-cache" && cachedResponsePromise))
          ) {
            try {
...
const fetchNetworkOnly = (event) =>
  event.respondWith(
    fetch(event.request).catch(() => console.log("Can't fetch from network"))
  );
    ...
    caches.open(cacheName).then((cache) =>
      fetch(event.request)
        .then(async (response) => {
          ...
          if (isCached) cache.put(event.request, response.clone());
          return response;
        })
        .catch(() => caches.match(event.request, { ignoreVary: true }))
    )
...
self.addEventListener("fetch", function (event) {
  if (isNetworkOnlyResource(event.request.url)) {
    ...
    } else fetchStaleWhenRevalidate(event, true);
  }
});
